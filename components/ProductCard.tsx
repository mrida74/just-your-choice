...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="group w-full overflow-hidden rounded-2xl border border-pink-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-pink-300 hover:shadow-xl sm:w-60 sm:flex-none">
      <Link href={`/category/${product.category}`} className="block">
        <div className="relative h-52 w-full bg-pink-50 sm:h-56">
          <Image
            src={image}
            alt={product.title}
            fill
